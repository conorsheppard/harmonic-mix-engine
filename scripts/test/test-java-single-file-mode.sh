#!/usr/bin/env bash
#
# Regression test for the `java-single-file-mode` make target.
#
# It runs `make java-single-file-mode`, captures the output, and checks that
# running HarmonicKeyMatcher.java in single-file source mode still produces its
# known-good results. Exits non-zero (with a diff-style report) if anything
# drifts.
#
# Usage: ./scripts/test/test-java-single-file-mode.sh
...
echo "==> Running 'make java-single-file-mode'..."
RAW_OUTPUT="$(make java-single-file-mode 2>&1)"
MAKE_STATUS=$?

if [[ $MAKE_STATUS -ne 0 ]]; then
  echo "FAIL: 'make java-single-file-mode' exited with status $MAKE_STATUS" >&2
  echo "----- output -----" >&2
  echo "$RAW_OUTPUT" >&2
  exit 1
fi
...
if [[ "$ACTUAL" == "$EXPECTED" ]]; then
  echo "PASS: java-single-file-mode output matches the expected result."
  exit 0
fi

echo "FAIL: java-single-file-mode output did not match expected." >&2
